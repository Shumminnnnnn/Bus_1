...
data class PlanInfo2(val markname: String, val formattedLongitude: String, val formattedLatitude: String)
object Plan_filter2 {
    suspend fun main(url: String): List<PlanInfo2> {
    ...
    private fun parseJson(jsonString: String): List<PlanInfo2> {
        val objectMapper = ObjectMapper()
        val rootNode = objectMapper.readTree(jsonString)
        val planInfoList = mutableListOf<PlanInfo2>()
        ...
        for (node in rootNode) {
            ...
            if (lonInRange && latInRange) {
                planInfoList.add(PlanInfo2(markname, formattedLongitude, formattedLatitude))
            }
        }
